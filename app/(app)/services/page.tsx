      ...
      <div className="max-w-screen-xl mx-auto px-4 pb-6 space-y-8">
        ...
        <section>
          ...
          <p className="text-sm text-muted-foreground mb-4">
            Login to the Online Services portal to get permits, pay taxes, or request official personal records
          </p>
          <div className="grid grid-cols-3 gap-3">
            {eServices.map((service) => (
              <Card 
                key={service.id}
                className="p-4 flex flex-col items-center text-center hover:bg-secondary/80 transition-colors cursor-pointer"
              >
        ...
        <section>
          ...
          <div className="space-y-3">
            {popularServices.map((service) => (
              <Card 
                key={service.id}
                className="p-4 flex items-center gap-4 hover:bg-secondary/80 transition-colors cursor-pointer"
              >
        ...
        <section>
          ...
          <div className="grid grid-cols-2 gap-3">
            {featuredServices.map((service) => (
              <Card 
                key={service.id}
                className="p-4 hover:bg-secondary/80 transition-colors cursor-pointer"
              >
                ...
                {expandedCategory === category.id && (
                  <div className="px-4 pb-4 space-y-2 bg-secondary/30">
                    {category.subServices.map((subService) => (
                      <div 
                        key={subService.id}
                        className="p-3 bg-card rounded-lg hover:bg-secondary/80 transition-colors cursor-pointer"
                      >
                        <h5 className="text-sm font-medium mb-1">{subService.title}</h5>
